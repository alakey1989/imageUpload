...
/**
     ...
     */
    public function crop($imgname,$type,$w, $h, $x = 0, $y = 0, $width = null, $height = null){
        ...
        empty($height) && $height = $h;
            ...
            //裁剪
            imagecopyresampled($img, $rec_img, 0, 0, $x, $y, $width, $height, $w, $h);
            imagedestroy($rec_img); //销毁原图
 ...
 /**
  * 
  * @param unknown $image	//原图路径
  * @param unknown $thumbname		//裁剪图的路径
  * @param string $type	//图片类型
  * @param number $maxWidth	//裁剪后保存图片的宽
  * @param number $maxHeight	//裁剪后保存图片的高
  * @param string $interlace
  * @param unknown $cut	//裁剪参数（x，y裁剪原点x，y坐标，w，h裁剪图片范围的宽高，compress_w，compress_h裁剪时缩放的宽和高）
  * @return unknown|boolean
  */
static function cropImg($image, $thumbname,$cropSize=array(300,300), $cut = array('x'=>0, 'y'=>0, 'compress_w'=>300, 'compress_h'=>300, 'w'=>300, 'h'=>'300'), $type='',$interlace=true) {
    	// 获取原图信息
    	$info = Image::getImageInfo($image);
    	if ($info !== false) {
    		$srcWidth = $info['width'];
    		...
    		$interlace = $interlace ? 1 : 0;
    		unset($info);
    		$maxWidth=$cropSize[0];	//裁剪后保存图片的宽
    		 $maxHeight=$cropSize[1];	//裁剪后保存图片的高
    		// 载入原图
    		$createFun = 'ImageCreateFrom' . ($type == 'jpg' ? 'jpeg' : $type);
    		$srcImg = $createFun($image);
    		...
    		//裁剪和原图宽高比例
    		$ratio_w = $cut['compress_w'] / $srcWidth;	
    		$ratio_h = $cut['compress_h'] / $srcHeight;
    		
    		 //裁剪坐标
    		$cropped_x = round($cut['x'] / $ratio_w);		
    		$cropped_y = round($cut['y'] / $ratio_h);
    		
    		//裁剪宽高
    		$cropped_w = round($cut['w'] / $ratio_w);
    		$cropped_h = round($cut['h'] / $ratio_h);
    		...
    		$cropped_image = imagecreatetruecolor($cropped_w, $cropped_h);
    		
    		
    		// 裁剪
    		imagecopy($cropped_image, $srcImg, 0, 0, $cropped_x, $cropped_y, $cropped_w, $cropped_h);
    		// 缩放
    		if (function_exists("ImageCopyResampled"))
    			imagecopyresampled($target_image, $cropped_image, 0, 0, 0, 0, $maxWidth, $maxHeight, $cropped_w, $cropped_h);
    		else 
    			imagecopyresized($target_image, $cropped_image, 0, 0, 0, 0, $maxWidth, $maxHeight, $cropped_w, $cropped_h);
    

    		
    		$thumbImg = $target_image;
